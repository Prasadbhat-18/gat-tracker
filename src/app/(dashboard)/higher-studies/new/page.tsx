...
import { HigherStudyForm } from "@/components/higher-studies/HigherStudyForm"
import type { Metadata } from "next"

export const metadata: Metadata = {
  title: "Record Higher Studies Progression",
}
...
export default async function NewHigherStudyPage({ searchParams }: Props) {
  const session = await auth()
  if (!session) redirect("/login")

  const canEdit = ["SUPER_ADMIN", "HOD", "FACULTY", "PLACEMENT_OFFICER"].includes(session.user.role)
  if (!canEdit) redirect("/higher-studies")
